...
public class AccountController {
	...
	public void refresh(ActionEvent e) {
		...
		try {
			txs = client.authPlayer.getTransactions();
		...
		BigDecimal balance;
		try {
			balance = client.authPlayer.getBalance();
		} catch (RemoteException e1) {
			e1.printStackTrace();
			return;
		}
	...
	public void fundAccount() {
		...
		String address;
		try {
			address = client.authPlayer.getBitcoinAddress();
	...
	public void withdraw() {
		...
		final Action submitAction = new AbstractAction("Withdraw") {
			...
			public void handle(ActionEvent ae) {
				...
				String txHash = "";
				try {
					txHash = client.authPlayer.withdraw(address, amount);
				} catch (RemoteException e) {
